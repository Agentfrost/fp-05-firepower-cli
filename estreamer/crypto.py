...
class Crypto(object):
        ...
        try:
            ...
            raise estreamer.EncoreException(
                definitions.STRING_PYOPENSSL_MISSING.format(
                    pkcs12Filepath, privateKeyFilepath, certificateFilepath
                )
            )

        try:
            # pkcs12 = OpenSSL.crypto.load_pkcs12( data, password )
            with open(pkcs12Filepath, "rb") as f:
                (private_key, certificate, additional_certificates) = (
                    pkcs12.load_key_and_certificates(f.read(), password.encode("utf-8"))
                )
    ...
    def create(settings=None, password=None, pkcs12Filepath=None):
        ...
        if settings is not None:
            if password is None:
                return Crypto(
                    settings.privateKeyFilepath(), settings.publicKeyFilepath()
                )
            ...
            Crypto.extract(
                pkcs12Filepath, password, privateKeyFilepath, certificateFilepath
            )
